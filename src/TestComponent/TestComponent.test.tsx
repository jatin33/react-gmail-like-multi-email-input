...
describe("EmailInput", () => {
  ...
  test("removing email", () => {
    const onEmailChange = jest.fn();
    render(<TestComponent emails={["test45@example.com"]} onEmailChange={onEmailChange} />);
  ...
  test("readonly mode", () => {
    const onEmailChange = jest.fn();
    render(<TestComponent emails={["test45@example.com"]} onEmailChange={onEmailChange} isDisabled />);
  ...
  test("handling paste event", () => {
    const onEmailChange = jest.fn();
    const { getByRole } = render(<TestComponent emails={[]} onEmailChange={onEmailChange} />);

    const input = getByRole("textbox");
    fireEvent.paste(input, { clipboardData: { getData: () => "test67@example.com" } });
  ...
  test("paste multiple emails at once", () => {
    const onEmailChange = jest.fn();
    const { getByRole } = render(<TestComponent delimiters={[";", ","]} emails={[]} onEmailChange={onEmailChange} />);

    const input = getByRole("textbox");
    fireEvent.paste(input, { clipboardData: { getData: () => "test67@example.com;jna@mail.com,ja@mail.com" } });

    expect(onEmailChange).toHaveBeenCalledWith(["test67@example.com"]);
    expect(onEmailChange).toHaveBeenCalledWith(["test67@example.com", "jna@mail.com"]);
    expect(onEmailChange).toHaveBeenCalledWith(["test67@example.com", "jna@mail.com", "ja@mail.com"]);
  });
